 ...
 * \param[in,out] stack la zone de pile concernée.
 */
void do_eq_prim(varray_t *stack, int structural) {
  int r = 0; // par défaut le résultat est faux

  // Tester si les types des arguments sont égaux
  if(structural) {
    if(varray_top_at(stack, 0)->type != varray_top_at(stack, 1)->type) { // Not the same type
      r = 0;
    } else {
      ...
      case T_BOOL:
      case T_INT:
        r = (value_int_get(varray_top_at(stack, 0)) == value_int_get(varray_top_at(stack, 1)));
        break;
    ...
    if((varray_top_at(stack,0)->type == T_INT || varray_top_at(stack,1)->type == T_INT)) {
      r = (value_int_get(varray_top_at(stack, 0)) == value_int_get(varray_top_at(stack, 1)));
    } else if((varray_top_at(stack,0)->type == T_BOOL || varray_top_at(stack,0)->type == T_INT) &&
              (varray_top_at(stack,1)->type == T_BOOL || varray_top_at(stack,1)->type == T_INT)) {
      r = (value_int_get(varray_top_at(stack, 0)) == 0 && value_int_get(varray_top_at(stack, 1)) == 0) ||
        (value_int_get(varray_top_at(stack, 0)) != 0 && value_int_get(varray_top_at(stack, 1)) != 0);
    } else {
