 ...
 *   implantation
 *
 * Les valeurs allouées par la VM sont les fermetures et les paires.
...
pair_t * gc_alloc_pair(vm_t *vm) {
  ...
  pair->gc_mark = (vm->gc->current_mark==0)? 1 : 0; // paire non-marquée initialement
...
env_t* gc_alloc_env(gc_t * gc, unsigned int capacity, env_t *next) {
  ...
  if(capacity == 0) {
    ...
    env->gc_mark = (gc->current_mark==0)? 1 : 0; // paire non-marquée initialement
